...
def generate_webtheme_css_file(path,sitename,self):
	try:
		...
		path = get_files_path()
		pages = frappe.db.get_all("Web Page Builder",filters={"published":1,"use_page_builder":1},fields=['route','name','header_component','footer_component','edit_header_style','is_transparent_header','menu_text_color','menu_hover_bg','menu_hover_text_color'])
		for page in pages:
			...
			if page.edit_header_style == 1:
				if page.is_transparent_header == 1:
					...
					webtheme_css+="."+page.route+" .go1-cms-menu li a:hover{background-color:"+page.menu_hover_bg+";color:"+page.menu_hover_text_color+";}"
					if header_settings:
